...
namespace sdds
{
    int no_of_traces; // holding number of records
    Customers *users;
    ...
    { // Do: complete the missing parts as guided (6 parts)
        ...
        if (openFile_r(filename_r))
        {
            ...
            if (i != no_of_traces) // i as a number of the records
            {
    ...
    { // Do: complete the missing parts as guided (3 parts)
        ...
        if (readDayOfWeek(&user_info.dayofweek) && readCustomerTimeUsing(&user_info.user_id) && readTimeInHours(&user_info.timeinhours) &&
            readCustomerTimeUsing(&user_info.dayofyear) && readCustomerTimeUsing(&user_info.Fwifitime) &&
            readCustomerTimeUsing(&user_info.Fctime) && readPackageName(read_Package_name))
        { // if reading of data
